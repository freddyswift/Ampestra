...
struct KEFCompanionApp: App {
    ...
    private var statusItemAccessibilityLabel: String {
        ...
        if appState.isReconnecting {
            return "KEF Companion, reconnecting"
        }
        if appState.discovery.isSearching {
            return "KEF Companion, searching for speakers"
        }
        if appState.connectionError != nil {
            return "KEF Companion, connection issue"
        }
        return "KEF Companion, no speaker connected"
    }
}
...
@MainActor
public func runKEFCompanionApp() {
    KEFCompanionApp.main()
}
...
@_cdecl("KEFCompanionDevMain")
@MainActor
public func kefCompanionDevMain() {
    KEFCompanionApp.main()
}
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    private let diagnosticLogger = Logger(
        subsystem: "com.freddyswift.KEFCompanion.dev",
    ...
    private func runDevelopmentNetworkDiagnosticIfRequested() {
        ...
        guard let host = ManualHostValidator.normalizedHost(suppliedHost) else {
            finishDevelopmentNetworkDiagnostic(
                message: "KEF_DIAGNOSTIC_INVALID_HOST\n",
                exitCode: 2
            )
            return
        }
        ...
        Task { @MainActor in
            do {
                ...
                finishDevelopmentNetworkDiagnostic(
                    message: "KEF_DIAGNOSTIC_OK host=\(host) name=\(name) model=\(model)\n",
                    exitCode: 0
                )
            } catch {
                let nsError = error as NSError
                finishDevelopmentNetworkDiagnostic(
                    message: "KEF_DIAGNOSTIC_FAILED domain=\(nsError.domain) code=\(nsError.code) description=\(nsError.localizedDescription)\n",
                    exitCode: 1
                )
            }
        }
