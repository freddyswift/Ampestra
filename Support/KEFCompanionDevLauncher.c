...
typedef void (*KEFCompanionEntryPoint)(void);
...
static int payload_path(char *destination, size_t capacity) {
    ...
    *last_separator = '\0';
    ...
    int written = snprintf(
        destination,
        capacity,
        "%s/../Frameworks/libKEFCompanionDevPayload.dylib",
        resolved_path
    );
...
int main(void) {
    ...
    if (!payload_path(path, sizeof(path))) {
        fputs("KEF Companion Dev could not locate its app payload.\n", stderr);
        return EXIT_FAILURE;
    }

    void *payload = dlopen(path, RTLD_NOW | RTLD_LOCAL);
    if (payload == NULL) {
        fprintf(stderr, "KEF Companion Dev could not load its app payload: %s\n", dlerror());
        return EXIT_FAILURE;
    }

    dlerror();
    KEFCompanionEntryPoint entry_point =
        (KEFCompanionEntryPoint)dlsym(payload, "KEFCompanionDevMain");
    const char *symbol_error = dlerror();
    if (symbol_error != NULL || entry_point == NULL) {
        fprintf(
            stderr,
            "KEF Companion Dev payload is missing its entry point: %s\n",
